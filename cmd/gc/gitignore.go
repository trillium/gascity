...
// skill materialization and must not be committed.
var cityGitignoreEntries = []string{
	".gc/", ".beads/*", "!.beads/config.yaml", "!.beads/metadata.json",
	"hooks/", ".runtime/",
	".claude/", ".codex/", ".gemini/", ".opencode/",
}

// rigGitignoreEntries are the paths that gc rig add writes into
// the rig-scoped .gitignore. Provider directories are included because
// skill materialization also targets rig scope roots.
var rigGitignoreEntries = []string{
	".beads/*", "!.beads/config.yaml", "!.beads/metadata.json",
	".claude/", ".codex/", ".gemini/", ".opencode/",
}
