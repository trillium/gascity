...
func TestCityGitignoreEntries_IncludesProviderDirs(t *testing.T) {
	// Provider directories (.claude/, .codex/, etc.) are created at runtime by
	// skill materialization. They must be in the city-level .gitignore so
	// users don't accidentally commit runtime-generated symlinks.
	required := []string{".claude/", ".codex/", ".gemini/", ".opencode/"}
	have := make(map[string]bool, len(cityGitignoreEntries))
	for _, e := range cityGitignoreEntries {
		have[e] = true
	}
	for _, want := range required {
		if !have[want] {
			t.Errorf("cityGitignoreEntries missing provider dir %q", want)
		}
	}
}

func TestRigGitignoreEntries_IncludesProviderDirs(t *testing.T) {
	// Provider directories are also materialized in rig scope roots.
	required := []string{".claude/", ".codex/", ".gemini/", ".opencode/"}
	have := make(map[string]bool, len(rigGitignoreEntries))
	for _, e := range rigGitignoreEntries {
		have[e] = true
	}
	for _, want := range required {
		if !have[want] {
			t.Errorf("rigGitignoreEntries missing provider dir %q", want)
		}
	}
}
